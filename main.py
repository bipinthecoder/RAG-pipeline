...
def generate_text(prompt, max_length = 1000):
    ...
    input_ids = inputs
    attention_mask = tf.cast(input_ids != gpt2_tokenizer.pad_token_id, dtype=tf.int32)

    if len(input_ids.shape) == 1:
        input_ids = tf.expand_dims(input_ids, 0)
        
    outputs = gpt2_model.generate(input_ids=input_ids, attention_mask=attention_mask,
...
def rag_system(user_query, index):
    ...
    combined_prompt = " ".join(relevant_documents_list) + "\nUser Request: " + user_query
